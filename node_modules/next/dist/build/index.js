...
async function build(dir, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false, appDirOnly = false, turboNextBuild = false) {
    ...
    try {
        ...
        const buildResult = await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            if (isAppDirEnabled) {
                if (!process.env.__NEXT_TEST_MODE && _ciinfo.hasNextSupport) {
                    const requireHook = require.resolve("../server/require-hook");
                    const contents = await _fs.promises.readFile(requireHook, "utf8");
                    await _fs.promises.writeFile(requireHook, `process.env.__NEXT_PRIVATE_PREBUNDLED_REACT = '${config.experimental.serverActions ? "experimental" : "next"}'\n${contents}`);
                }
            }
