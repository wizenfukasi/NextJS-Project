...
import { DOT_NEXT_ALIAS, PAGES_DIR_ALIAS, ROOT_DIR_ALIAS, APP_DIR_ALIAS, WEBPACK_LAYERS, RSC_ACTION_PROXY_ALIAS, RSC_ACTION_CLIENT_WRAPPER_ALIAS } from "../lib/constants";
...
const asyncStoragesRegex = /next[\\/]dist[\\/]client[\\/]components[\\/](static-generation-async-storage|action-async-storage)/;
...
export default async function getBaseWebpackConfig(dir, { buildId , config , compilerType , dev =false , entrypoints , isDevFallback =false , pagesDir , reactProductionProfiling =false , rewrites , originalRewrites , originalRedirects , runWebpackSpan , appDir , middlewareMatchers , noMangling =false , jsConfig , resolvedBaseUrl , supportedBrowsers , clientRouterFilters , previewModeId , fetchCacheKeyPrefix , allowedRevalidateHeaderKeys  }) {
    ...
    async function handleExternals(context, request, dependencyType, layer, getResolve) {
        ...
        if (!isLocal) {
            ...
            const notExternalModules = /^(?:private-next-pages\/|next\/(?:dist\/pages\/|(?:app|document|link|image|legacy\/image|constants|dynamic|script|navigation|headers)$)|string-hash|private-next-rsc-action-proxy|private-next-rsc-action-client-wrapper$)/;
                        ...
                        oneOf: [
                            ...
                            },
                            {
                                ...
                                resolve: {
                                    alias: {
                                        react: `next/dist/compiled/react${bundledReactChannel}`,
                                        "react-dom$": reactProductionProfiling ? `next/dist/compiled/react-dom${bundledReactChannel}/cjs/react-dom.profiling.min` : `next/dist/compiled/react-dom${bundledReactChannel}`,
