...
export default async function build(dir, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false, appDirOnly = false, turboNextBuild = false) {
    let hasAppDir = false;
    try {
        ...
        const buildResult = await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            if (isAppDirEnabled) {
                if (!process.env.__NEXT_TEST_MODE && ciEnvironment.hasNextSupport) {
                    ...
                    await promises.writeFile(requireHook, `process.env.__NEXT_PRIVATE_PREBUNDLED_REACT = '${config.experimental.serverActions ? "experimental" : "next"}'\n${contents}`);
                }
            }
            ...
            rewrites.beforeFiles.unshift(...generateInterceptionRoutesRewrites(appPageKeys));
            ...
            const numWorkers = Math.max(config.experimental.cpus !== defaultConfig.experimental.cpus ? config.experimental.cpus : Math.min(config.experimental.cpus || 1, Math.floor(os.freemem() / 1e9)), // enforce a minimum of 4 workers
            4);
